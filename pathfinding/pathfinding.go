...
func NewPathFinder(mmapMgr *MMapManager, terrainMgr *TerrainManager, vmapMgr *VMapManager, mapID uint32) (*PathFinder, error) {
	md, err := mmapMgr.getOrLoadMapData(mapID)
	if err != nil {
		return nil, err
	}
...
func (pf *PathFinder) navMeshRLock() {
	if pf.meshMu != nil {
		pf.meshMu.RLock()
	}
}

func (pf *PathFinder) navMeshRUnlock() {
	if pf.meshMu != nil {
		pf.meshMu.RUnlock()
	}
}
...
func (pf *PathFinder) FindPath(start, dest Point3D) (*PathResult, error) {
	...
	for i := range points {
		if r, ok := pf.getNearestPolyHeightLocked(points[i].X, points[i].Y, points[i].Z+0.5, 20); ok {
			points[i].Z = r
		}
	}
...
// vmap collision first, terrain next, then navmesh poly as a fallback.
...
// GetTerrainHeight returns the raw map grid height at (x,y), without using the
// caller's Z hint or navmesh poly height. Bot movement uses this for final path
// Z snapping so downhill terrain can pull dense wander samples down.
func (s *PathFindingService) GetTerrainHeight(mapID uint32, x, y float32) (float32, bool) {
	if s.terrainMgr == nil {
		return 0, false
	}
	return s.terrainMgr.GetGridHeight(mapID, x, y)
}
...
func (pf *PathFinder) getMapHeight(x, y, z float32) (float32, bool) {
	...
	pf.navMeshRLock()
	defer pf.navMeshRUnlock()
	if ph, ok := pf.getNavMeshHeightAtOrBelowLocked(x, y, searchZ, DEFAULT_HEIGHT_SEARCH); ok {
		return ph, true
	}
	return 0, false
}

func (pf *PathFinder) getNavMeshHeightAtOrBelowLocked(x, y, searchZ, maxSearch float32) (float32, bool) {
	ph, ok := pf.getNearestPolyHeightLocked(x, y, searchZ, maxSearch)
	if !ok {
		return 0, false
	}
	if ph > searchZ+GROUND_HEIGHT_TOLERANCE {
		return 0, false
	}
	if searchZ-ph > maxSearch+GROUND_HEIGHT_TOLERANCE {
		return 0, false
	}
	return ph, true
}

func (pf *PathFinder) getNearestPolyHeightLocked(x, y, z, verticalExtent float32) (float32, bool) {
	pt := [3]float32{y, z, x}
	var pr detour.DtPolyRef
	var cl [3]float32
	ext := [3]float32{5, verticalExtent, 5}
	if st := pf.query.FindNearestPoly(pt[:], ext[:], pf.filter, &pr, cl[:]); detour.DtStatusFailed(st) || pr == invalidPolyRef {
		return 0, false
	}

	var ph float32
	if st := pf.query.GetPolyHeight(pr, cl[:], &ph); detour.DtStatusFailed(st) {
		return 0, false
	}
	return ph, true
}

// GetHeight returns the ground Z at (x,y), preferring vmap collision when available.
...
func (pf *PathFinder) GetHeight(x, y, z float32) (float32, bool) {
	...
	return pf.getHeightLocked(x, y, z)
}

func (pf *PathFinder) getHeightLocked(x, y, z float32) (float32, bool) {
	...
	// Last resort poly query. It must be at or below searchZ, matching a downward
	// height query. Probing upward can select higher navmesh layers and then feed
	// that Z back into bot movement.
	if pH, ok := pf.getNavMeshHeightAtOrBelowLocked(x, y, searchZ, DEFAULT_HEIGHT_SEARCH); ok {
		return pH, true
	}
	return 0, false
