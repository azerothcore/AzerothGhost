...
-- For advanced: use scripts/ai/init.lua (see ai/README.md) which provides better generic + class libs via S/T/A/V.
-- optionally: local ai = dofile("scripts/ai/init.lua"); ai:enable_default_strategies(); function on_tick() ai:Tick() end
-- (auto wires warrior/hunter/mage based on bot.get_class())
...
function on_tick()
    ...
    if target then
        ...
        elseif target.distance > 5 then
            ...
            bot.stop_moving()
            if bot.set_target then pcall(function() bot.set_target(target.guid) end) end
